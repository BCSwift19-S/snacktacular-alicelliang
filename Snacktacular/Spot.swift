    ...
    func updateAverageRating(completed: @escaping () -> () ) {
        ...
        reviewsRef.getDocuments { (querySnapshot, error) in
            guard error == nil else {
                print("**** ERROR: failed to get querysnapshot review for reviewRef: \(reviewsRef.path), error: \(error!.localizedDescription)")
                return completed()
            }
            var ratingTotal = 0.0
            for document in querySnapshot!.documents { //go thru all of the review documents
            ...
            spotRef.setData(dataToSave) { error in // save it and check errors
                guard error == nil else {
                    print("*** ERROR: updating document \(self.documentID) in spots after changing average reviews and number of reviews, error: \(error!.localizedDescription)")
                    return completed()
                }
